...
<head>
    ...
    <style>
        ...
        .btn-buy { background: linear-gradient(45deg, #28a745, #20c997); color: white; width: 100%; padding: 10px; border: none; border-radius: 8px; font-weight: 900; font-size: 12px; letter-spacing: 1px; cursor: pointer; transition: 0.2s; text-decoration: none; display: inline-block; text-align: center; box-sizing: border-box; }
...
<body>
    ...
    <div id="page-home" class="page-section active">
        ...
        <div class="card">
            ...
            <a href="{{ i.link }}" target="_blank" class="btn-buy">UNLOCK PREMIUM</a>
        </div>
        {% endfor %}
    </div>
    ...
    <div id="p" class="popup">
        <div class="box" id="content-box">
            ...
            <div id="popup-inner">
                ...
                <p style="font-size:11px; color:#aaa; margin:10px 0;">Contact admin after payment to add funds.</p>
                <button class="btn-buy" onclick="window.open('https://t.me/Skjsbsh166', '_blank')">CONTACT ADMIN</button>
            </div>
        </div>
    </div>
...
def home():
    ...
    return render_template_string(HTML, content=content, total_wallet=total_wallet, total_orders=total_orders, completed_orders=completed_orders, user_id=session['user_id'])

# UptimeRobot ke liye 24/7 active rakhne wala ping route
@app.route('/ping')
def ping():
    return "OK", 200
