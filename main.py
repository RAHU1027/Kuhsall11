...
<head>
    ...
    <style>
        ...
        .header { position:fixed; top:0; width:100%; background:#15253d; padding:15px; display:flex; justify-content:space-between; z-index:1000; border-bottom:1px solid #007bff; align-items:center; box-sizing:border-box; }
        ...
        .btn-demo { background:#ffc107; color:black; padding:10px; margin-bottom:10px; border-radius:5px; font-weight:bold; text-align:center; cursor:pointer; display:block; }
...
<body>
    <div class="header">
        ...
        <div style="font-weight:bold;">💰 Wallet: ₹0</div>
    </div>
    {% for i in content %}
    <div class="card">
        <img src="{{ i.media }}">
...
def verify_payment():
    ...
    if not os.path.exists("verified_ids.txt"): open("verified_ids.txt", "w").close()
    with open("verified_ids.txt", "r") as f:
        if tid in f.read(): return jsonify({"message": "Yeh Transaction ID pehle hi use ho chuki hai!", "status": "error"})
    
    msg = f"🔔 *New Order*\n💰 Amt: ₹{amt}\n🆔 ID: `{tid}`\n\n[✅ VERIFY KUSHAL]({DOMAIN}/admin-verify/{tid})"
